...
// - CALLBACK_TOKEN : e.g. someStrongSecret123
...
const toCents = v => {
  ...
  if (/^\d+$/.test(str)) return parseInt(str, 10);     // looks like already cents
  const f = Number.parseFloat(str);
  if (Number.isNaN(f)) return 0;
  return Math.round(f * 100);                           // treat as RM
};

// ToyyibPay usually posts form-urlencoded
...
// CALLBACK URL (server-to-server) with token + validation + UPSERT
app.post("/toyyib/callback/:token", async (req, res) => {
  try {
    ...
    // 2) Read payload (ToyyibPay can send form or json)
    ...
    if (!order_id || !status_id) {
      console.warn("Missing required fields:", { order_id, status_id });
    ...
    // 5) Optional validation: if order exists and amount mismatched → keep but mark text
